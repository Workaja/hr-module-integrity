...
$getInfoClaim = 
				$ki->query("select 
				(select ifnull(sum(total_date),0) from lembur_log where escalated_to = '".$_SESSION['fusn']."') as allvpclaim,
				(select ifnull(sum(total_date),0) from lembur_log where escalated_to = '".$_SESSION['fusn']."' and status = 'DRAFT') as draftvpclaim,
				(select ifnull(sum(total_date),0) from lembur_log where escalated_to = '".$_SESSION['fusn']."' and status = 'APPROVED') as approvedvpclaim,
				(select ifnull(sum(total_date),0) from lembur_log where escalated_to = '".$_SESSION['fusn']."' and status = 'REJECT') as rejectedvpclaim
				");
$CInfo = $getInfoClaim->fetch_assoc();

$getVarPayClaimDataDraft = $ki->query("select a.no_claim as no_claim, b.name as name, a.tgl_input as tgl_input, a.total_date as total_date, a.status as status, a.tgl_start as tgl_start, a.tgl_end as tgl_end from lembur_log a inner join users b on a.user = b.username where a.status = 'DRAFT' and a.escalated_to = '".$_SESSION['fusn']."'");

$getVarPayClaimDataApproved = $ki->query("select a.no_claim as no_claim, b.name as name, a.tgl_input as tgl_input, a.total_date as total_date, a.status as status, a.tgl_start as tgl_start, a.tgl_end as tgl_end from lembur_log a inner join users b on a.user = b.username where a.status = 'APPROVED' and a.escalated_to = '".$_SESSION['fusn']."'");

$getVarPayClaimDataRejected = $ki->query("select a.no_claim as no_claim, b.name as name, a.tgl_input as tgl_input, a.total_date as total_date, a.status as status, a.tgl_start as tgl_start, a.tgl_end as tgl_end from lembur_log a inner join users b on a.user = b.username where a.status = 'REJECT' and a.escalated_to = '".$_SESSION['fusn']."'");
?>
...
if(isset($_SESSION['vpclaiminfo'])){
	switch($_SESSION['vpclaiminfo']) {
		...
		case 'Dollar':
		echo "<div class='row'>";
		echo "<div class='col-md-12'><div class='alert alert-success' role='alert'><a href='#' class='alert-link'>".$_SESSION['vpclaiminfo_detail']."</a> Successfully Update Dollar Conversion.</div></div>";
		echo "</div>";
		unset($_SESSION['vpclaiminfo']);
		unset($_SESSION['vpclaiminfo_detail']);
		break;
