...
while($so_list = $get_so_list->fetch_assoc()) {
	...
	$get_cuti_log = $ki->query("select total_date, tgl_start, tgl_end, tgl_list, jenis_lembur, dollar from lembur_log where user = '".$so_list['username']."' and status = 'APPROVED'");
	...
	$arrayDollar = array_filter(explode(":", $yyy));

	$aDo = "";
	foreach($arrayDollar as $aD) {
		$aDo .= "'" . $aD . "',";
	}

	$DollarList = substr($aDo, 0, -1);

	if($dua != 0) { 
		echo $dua; 
		$setDollarDua = $ki->query("select ifnull(sum(konversi),0) as Final from daily_dollar where tanggal in (".$DollarList.");");
		$DollarDua = $setDollarDua->fetch_assoc();
		$FinalDollar = (int) $DollarDua['Final'] * 25;
	} else {
		$FinalDollar = 0;
	}
	///////////////////////////////////////////////
