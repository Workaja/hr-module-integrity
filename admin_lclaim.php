...
$getInfoClaim = 
				$ki->query("select 
				(select ifnull(sum(totaldate),0) from cuti_log where escalated_to = '".$_SESSION['fusn']."') as alllclaim,
				(select ifnull(sum(totaldate),0) from cuti_log where escalated_to = '".$_SESSION['fusn']."' and status = 'DRAFT') as draftlclaim,
				(select ifnull(sum(totaldate),0) from cuti_log where escalated_to = '".$_SESSION['fusn']."' and status = 'APPROVED') as approvedlclaim,
				(select ifnull(sum(totaldate),0) from cuti_log where escalated_to = '".$_SESSION['fusn']."' and status = 'REJECTED') as rejectedlclaim");
$CInfo = $getInfoClaim->fetch_assoc();

$getLeaveClaimDataDraft = $ki->query("select a.no_claim as no_claim, b.name as name, a.tgl_input as tgl_input, a.totaldate as totaldate, a.status as status, a.tgl_cuti_from as tgl_cuti_from, a.tgl_cuti_to from cuti_log a inner join users b on a.user = b.username where a.status = 'DRAFT' and a.escalated_to = '".$_SESSION['fusn']."'");

$getLeaveClaimDataApproved = $ki->query("select a.no_claim as no_claim, b.name as name, a.tgl_input as tgl_input, a.totaldate as totaldate, a.status as status, a.tgl_cuti_from as tgl_cuti_from, a.tgl_cuti_to from cuti_log a inner join users b on a.user = b.username where a.status = 'APPROVED' and a.escalated_to = '".$_SESSION['fusn']."'");

$getLeaveClaimDataRejected = $ki->query("select a.no_claim as no_claim, b.name as name, a.tgl_input as tgl_input, a.totaldate as totaldate, a.status as status, a.tgl_cuti_from as tgl_cuti_from, a.tgl_cuti_to from cuti_log a inner join users b on a.user = b.username where a.status = 'REJECTED' and a.escalated_to = '".$_SESSION['fusn']."'");
?>
